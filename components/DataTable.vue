  ...
  <table id="data-table" class="pa4 f6 w-100">
    ...
    <tbody v-if="sortedDataByGrainType">
      <tr
        ...
        :key="oblast.oblastNameEnglish"
      >
      <!-- <pre class="h3 overflow-y-auto ba b--red">{{oblast}}</pre> -->
        <td class="w-20">{{ oblast.oblastNameEnglish }}</td>
        ...
        <td class="tl w-20">
          <div class="slider-cell">
            <input
              ...
              class="slider"
              @change="
                setOblastScale($event.target.value, oblast.oblastNameEnglish)
              "
              :value="getOblastPercentage(oblast.oblastNameEnglish)"
              :id="`range-${oblast.oblastNameEnglish}`"
            />
            {{ getOblastPercentage(oblast.oblastNameEnglish) }}%
          </div>
        </td>
      </tr>
    </tbody>
